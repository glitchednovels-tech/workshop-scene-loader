...
- **Area attacks** (grenades, cannon lines, suppressive fire): **Place template** puts an orange circle, square or line on the map. Lines start at the caster; select a token first to aim at it, or use **Aim at selected token** / **Rotate to °**. Drag it where you want. For instant areas (beams, squares, bursts), targeting is live: everything touching the template becomes a target, marked **in area**, and anything moved out of it is dropped again, unless its damage was already applied or you added it by hand. Friendly fire is on by default. Lasting areas like the grenade become active as soon as they're placed: the **Active areas** card (Turn and Order tabs) shows who is inside right now, **Roll for everyone inside now** asks for saves the moment it lands, and anything starting its turn inside gets a roll prompt until it runs out (the template is removed then).
